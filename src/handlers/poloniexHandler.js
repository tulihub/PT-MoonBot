...
class PoloniexHandler {
    ...
    handlePoloniexMessage(data) {
        let orders = [];
        let balances = [];
        let markets = [];

        try {
            orders = Object.assign({}, data.poloniex.orders);
            balances = Object.assign({}, data.poloniex.balances);
            markets = Object.assign({}, data.poloniex.markets);
        } catch (err) {
            logger.debug("error handling poloniex message: " + err);
        }
        ...
        for (let curr in balances) {
            ...
            // let currentMarket = markets[curr] ? markets[curr] :
            if (curr == moonBotMarket) {
                ...
                try {
                    ...
                    logger.warn("problem with loading poloniex balance of " + curr)
                }
            }
        }
...
module.exports = PoloniexHandler;